...
import DeepSTN_metrics as metrics
...
def conv_unit0(Fin,Fout,drop,H,W):
    ...
    print('unit_conv', unit_conv.shape)
    unit_conv = tf.transpose(unit_conv, (0, 2, 3, 1))
    unit_output=Conv2DFilterPool(num_outputs=Fout)(unit_conv)
    unit_output = tf.transpose(unit_output, (0, 3, 1, 2))
    print('unit_output', unit_output.shape)
    unit_model=Model(inputs=unit_input,outputs=unit_output)
    print('kernel=(hex,hex)')
    return unit_model
...
def PT_trans(name,P_N,PT_F,T,T_F,H,W,isPT_F):
    if 1:
        ...
        time_in=Input(shape=(T,H,W))

        T_xs = []
        for _ in range(P_N):
            T_xs.append(T_trans(T,T_F,H,W)(time_in))
        T_x=Concatenate(axis=1)(T_xs)
...
# main model
def DeepSTN(H=29,W=14,channel=2,      #H-map_height W-map_width channel-map_channel
            c=6,p=7,t=3,              #c-closeness p-period t-trend
