...
from lib.data_preparation import read_and_generate_dataset2 as read_and_generate_dataset
from model.model_config import get_backbones2 as get_backbones
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):
        ...
        print('epoch time: ', time() - tot_train_time)
        ...
        prediction_path = training_config['prediction_filename']
        prediction = predict(net, test_loader)
        newtest_dir = f'daejeon_result'
        if not os.path.isdir(newtest_dir):
            os.mkdir(newtest_dir)
        np.save(f'{newtest_dir}/label_{prediction_path}.npy', all_data['test']['target'])
        np.save(f'{newtest_dir}/{prediction_path}.npy', prediction)
